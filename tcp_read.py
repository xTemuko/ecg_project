...
ADC_VOLTAGE_FULL_SCALE_POSITIVE = 4.096 
...
def setup_plot_axes():
    ...
    ax_raw.set_title("ECG Original")
    ax_raw.set_ylabel("Voltaje (V)") 
    ax_raw.grid(True) 

    ax_denoised.clear()
    ax_denoised.set_ylim(-4, 4) 
    ax_denoised.set_title("ECG Filtrado ")
    x_tick_interval = DOWNSAMPLED_SAMPLES // 10
    ax_denoised.set_xlabel(f"Muestras (intervalo de {x_tick_interval} muestras)")
    ax_denoised.set_ylabel("Amplitud Normalizada (Z-score)") 
    ...
    if new_data_received and model_unet is not None:
        try:
            ...
            if std_val > 1e-6: 
                ...
                input_data_model_normalized = latest_ecg_data_volts - mean_val 
